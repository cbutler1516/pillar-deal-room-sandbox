...
describe("playbook registry", () => {
  ...
  it("maps sandbox loan types to baseline playbooks", () => {
    expect(baselinePlaybookKeysForLoanType("Fix & Flip")).toEqual([
      ...
      "request_preliminary_title_report",
      "prepare_submission",
    ]);
    expect(baselinePlaybookKeysForLoanType("DSCR Purchase")).toEqual([
      "request_government_id",
    ...
    expect(baselinePlaybookKeysForLoanType("Ground-Up Construction")).toContain(
      "request_construction_plans",
    );
    expect(baselinePlaybookKeysForLoanType("Bridge")).toContain(
      "request_preliminary_title_report",
    );
